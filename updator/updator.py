...
json_path = "resources/system_values.json"
...
def main():
    ...
    if ret:
        ...
        latest_ver_str = (
            "v" + latest_ver_str[0] + "." + latest_ver_str[1] + "." + latest_ver_str[2]
        )
        if (
            sg.PopupYesNo(
                f"新しいバージョン {latest_ver_str} が見つかりました。アップデートの内容は以下の通りです。\n\n\n{release_title}\n\n{release_body}\n\n\nアップデートを行います。よろしいですか？",
                title="アテンダン アップデーター",
                keep_on_top=True,
            )
            == "Yes"
            ...
            if isinstance(ret, Exception):
                sg.PopupOK(
                    f"アップデートに失敗しました。アップデートファイルのコピーに失敗しました。\n"
                    + str(ret),
                    title="アテンダン アップデーター",
                    keep_on_top=True,
                )
...
def get_latest_release_info():
    ...
    try:
        ...
        sg.Popup(
            f"アップデートの確認に失敗しました。インターネット接続を確認し，１時間ほど時間をおいて，再度試してください。\n"
            + str(e),
            title="アテンダン アップデーター",
            keep_on_top=True,
        )
...
def replace_with_latest_release(release_dir_path="AttenDan_release"):
    ...
    for item in os.listdir("."):
        if (
            item != "old_files"
            and item != "updator"
            and item != "updator.exe"
            and item != "AttenDan_release"
            and item != "saves"
            and item != ".git"
            and item != "unins000.exe"
            and item != "unins000.dat"
        ):  # 一部のファイルは削除しない
            try:
    ...
    for item in os.listdir(path):
        if (
            item != "saves" and item != ".git" and item != "updator.exe"
        ):  # 一部ファイルはコピーしない
            try:
